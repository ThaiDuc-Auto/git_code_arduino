...
{
  for(int d3=d2-24;d3<=d2;d3++)
  {
    for(int d4=d1;d4<=d1+(12*set);d4++)             ///xiu co the sua cho nay nha
    {
      matrix.drawPixel(d4,d3,0);
    }
  }
}
void clear_vitri2(int d1,int d2,int set)
{
...
void clear_vitri3(int d1,int d2,int set)
{
...
{
  for(int d3=d2-2;d3<=d2;d3++)
  {
    for(int d4=d1;d4<=d1+(3*set);d4++)             ///xiu co the sua cho nay nha
    {
      matrix.drawPixel(d4,d3,0);
    }
  }
...
// void clear_set(int x,int y,int nhay_so,int chieu_cao)        // ta tien hanh set nhieu so va nhieu ham nha pro
// {
//   for(int n=y-chieu_cao;n<=y;n++)
//   {
//     for(int c=x;c<((x+(ht_tmk_a(nhay_so)+1)*6)+1);c++)             ///xiu co the sua cho nay nha
//     {
//       matrix.drawPixel(c,n,0);
//     }
//   }
// }
