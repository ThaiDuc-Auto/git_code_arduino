...
void loop() {
  ...
  matrix.setTextSize(1); 
   matrix.setFont(&Font_Thai_2_1panel_LED7_17);
    ...
    if(tong_sp!=((int(plc.ReadWord(5))*65536+int(plc.ReadWord(4))))&((int(plc.ReadWord(5))*65536+int(plc.ReadWord(4)))<1000000))
    {
      tong_sp=(int(plc.ReadWord(5))*65536+int(plc.ReadWord(4)));
    }
    
  // if(che_do==0)
  // {
       if(dem_set!=int(plc.ReadWord(2)))
        {
          dem_set=int(plc.ReadWord(2));
          dem=dem_set; 
        }
   //}
   // //////////***********************
