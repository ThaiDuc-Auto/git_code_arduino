...
public:
    ModbusRS(uint8_t u8id, Stream& port, uint8_t u8txenpin =0);
    ...
    ModbusRS(uint8_t u8id=0, uint8_t u8serno=0, uint8_t u8txenpin=0) __attribute__((deprecated));
 ...
 * @ingroup setup
 */
ModbusRS::ModbusRS(uint8_t u8id, Stream& port, uint8_t u8txenpin)
{
    this->port = &port;
    this->u8id = u8id;
 ...
 */
ModbusRS::ModbusRS(uint8_t u8id, uint8_t u8serno, uint8_t u8txenpin)
{
    this->u8id = u8id;
    this->u8txenpin = u8txenpin;
    this->u16timeOut = 1000;
 ...
 */
template<typename T_Stream>
void ModbusRS::begin(T_Stream* install_port, long u32speed)
{
    port = install_port;
 ...
 */
template<typename T_Stream>
void ModbusRS::begin(T_Stream* install_port, long u32speed, uint8_t u8txenpin)
{
    this->u8txenpin = u8txenpin;
 ...
 * @ingroup setup
 */
void ModbusRS::begin(long u32speed)
{
 ...
 * @ingroup setup
 */
void ModbusRS::setID( uint8_t u8id)
{
    if (( u8id != 0) && (u8id <= 247))
    {
        this->u8id = u8id;
 ...
 * @ingroup setup
 */
void ModbusRS::setTxendPinOverTime( uint32_t u32overTime )
{
 ...
 * @ingroup setup
 */
void ModbusRS::setTimeOut( uint16_t u16timeOut)
{
 ...
 * @ingroup loop
 */
boolean ModbusRS::getTimeOutState()
{
 ...
 */
int8_t ModbusRS::query( modbus_t telegram )
{
 ...
 * @ingroup loop
 */
int8_t ModbusRS::poll( uint16_t *regs, uint8_t u8size )
{
 ...
 * @ingroup buffer
 */
uint16_t ModbusRS::calcCRC(uint8_t u8length)
{
 ...
 * @ingroup buffer
 */
uint8_t ModbusRS::validateRequest()
{
 ...
 * @ingroup buffer
 */
uint8_t ModbusRS::validateAnswer()
{
    // check message crc vs calculated crc
    uint16_t u16MsgCRC =
        ((au8Buffer[u8BufferSize - 2] << 8)
 ...
 * @ingroup buffer
 */
void ModbusRS::buildException( uint8_t u8exception )
{
 ...
 * @ingroup discrete
 */
int8_t ModbusRS::process_FC1( uint16_t *regs, uint8_t /*u8size*/ )
{
 ...
 * @ingroup register
 */
int8_t ModbusRS::process_FC3( uint16_t *regs, uint8_t /*u8size*/ )
{
 ...
 * @ingroup discrete
 */
int8_t ModbusRS::process_FC5( uint16_t *regs, uint8_t /*u8size*/ )
{
 ...
 * @ingroup register
 */
int8_t ModbusRS::process_FC6( uint16_t *regs, uint8_t /*u8size*/ )
{
 ...
 * @ingroup discrete
 */
int8_t ModbusRS::process_FC15( uint16_t *regs, uint8_t /*u8size*/ )
{
 ...
 * @ingroup register
 */
int8_t ModbusRS::process_FC16( uint16_t *regs, uint8_t /*u8size*/ )
{
